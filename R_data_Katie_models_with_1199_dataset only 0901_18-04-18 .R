...
##1 linear relationship between independent and dependent variables (or log(odds) for logistic regression) (ignore for categorical variables)

###malaria (plot of log(odds) against each variable for raw data)
plot(log())
###distance
plot(data4$Age,log(data4$distance_out)) #ignore
...
ggplot()+geom_density(data = data4 , aes(x=log(distance_urban_rural_km))) ##bimodal - remove?

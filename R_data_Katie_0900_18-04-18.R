...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan")
test<-multinom(Month.travel_relevel~ as.factor(age_class), data = data4) 
summary(test)
coef.test<-coef(test) #stand alone coefficients of model 
summary(test)$standard.errors #stand alone standard errors of model
exp.test<-exp(coef(test))
confint(test,level = 0.95)
conf.test<-exp(confint(test))
test_zvalues<-summary(test)$coefficients/summary(test)$standard.errors #gives z values
test_zvalues
test_pvalues<-pnorm(abs(test_zvalues), lower.tail=FALSE)*2#gives p values 
test_pvalues
Signif <- symnum(test_pvalues, corr = FALSE, na = FALSE, cutpoints = c(0, 
      0.001, 0.01, 0.05, 0.1, 1), symbols = c("***", "**", "*", ".", " "), legend = TRUE )
paste(test_pvalues,Signif, sep = " ")
...
time_occupation.OUT<-glm(Month.travel ~ data4_selected_occupations, data = data4, family=binomial(link=logit))
time_occupation.OUT
...
stargazer (test, 
           type = "html", title = "Coefficients standard error and significance of t / z values for univariate logistic models of variables affecting distance", style = "default", 
           summary = NULL, out = "test.html", out.header = FALSE,
           column.labels = NULL, column.separate = NULL,
           covariate.labels = NULL, dep.var.caption = "Distance models",
           dep.var.labels = NULL, dep.var.labels.include = FALSE,
           notes = "(1) distance ~ age, (2) distance ~ occupation, (3) distance ~ gender, (4) distance ~ reason for travel (5) distance ~ travel means (6) distance ~ border post (7) distance ~ home population (8) distance ~ home distance to nearest urban centre (9) distance ~ travel time of year", notes.align = "l") 

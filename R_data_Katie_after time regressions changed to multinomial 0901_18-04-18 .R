...
malaria_travel_prevalence.OUT<-glm(as.factor(Malaria_0_1_prevalence_exists) ~ data4_TYPE1or2, data = data4, family=binomial(link=logit)) # malaria modelled by travel_prevalence
...
travel_prevalence_occupation.OUT<-glm(falciparum_travel_prevalence ~ data4_selected_occupations, data = data4) 
...
travel_prevalence_ReasonTravel.OUT<-glm(falciparum_travel_prevalence ~ data4_selected_ReasonTravel, data = data4) 
...
travel_prevalence_MeansTravel.OUT<-glm(falciparum_travel_prevalence ~ data4_selected_MeansTravel, data = data4) 
...
travel_prevalence_BorderPost.OUT<-glm(falciparum_travel_prevalence ~ data4_selected_BorderPost, data = data4) 
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time_occupation.OUT<-multinom(Month.travel_relevel~ data4_selected_occupations, data = data4) 
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time_gender.OUT<-multinom(Month.travel_relevel~ data4_selected_genders, data = data4) 
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time_reason_travel.OUT<-multinom(Month.travel_relevel~ data4_selected_ReasonTravel, data = data4) 
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time_means_travel.OUT<-multinom(Month.travel_relevel~ data4_selected_MeansTravel, data = data4) 
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time_border_post.OUT<-multinom(Month.travel_relevel~ data4_selected_BorderPost, data = data4) 
...
travel_prevalence.OUT<- glm(falciparum_travel_prevalence ~ data4_selected_occupations + data4_selected_genders + data4_selected_ReasonTravel + data4_selected_BorderPost + pop.HOME + HOME_urban_rural_distance..degrees. , data=data4)
...
Month.travel_relevel<-relevel(data4$Month.travel.name, ref="Jan") # base month against which to compare probability of travel during each other month (for each variable, e.g. each age class)
time.OUT<-multinom(Month.travel_relevel~ as.factor(age_class) + data4_selected_occupations + data4_selected_genders + data4_selected_ReasonTravel + data4_selected_MeansTravel + data4_selected_BorderPost + pop.HOME + HOME_urban_rural_distance..degrees., data = data4) 
summary(time.OUT)

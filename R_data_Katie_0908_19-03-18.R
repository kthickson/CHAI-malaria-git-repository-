...
data4_students_Jan<-subset(data4_students,Month.travel==1)
length(data4_students_Jan$ID)
data4_students_Feb<-subset(data4_students,Month.travel==2)
length(data4_students_Feb$ID)
data4_students_Mar<-subset(data4_students,Month.travel==3)
length(data4_students_Mar$ID)
data4_students_Apr<-subset(data4_students,Month.travel==4)
length(data4_students_Apr$ID)
data4_students_May<-subset(data4_students,Month.travel==5)
length(data4_students_May$ID)
data4_students_Jun<-subset(data4_students,Month.travel==6)
length(data4_students_Jun$ID)
data4_students_Jul<-subset(data4_students,Month.travel==7)
length(data4_students_Jul$ID)
data4_students_Aug<-subset(data4_students,Month.travel==8)
length(data4_students_Aug$ID)
data4_students_Sep<-subset(data4_students,Month.travel==9)
length(data4_students_Sep$ID)
data4_students_Oct<-subset(data4_students,Month.travel==10)
length(data4_students_Oct$ID)
data4_students_Nov<-subset(data4_students,Month.travel==11)
length(data4_students_Nov$ID)
data4_students_Dec<-subset(data4_students,Month.travel==12)
length(data4_students_Dec$ID)

library(ggplot2)
student_seasonal_travel_frequency<- ggplot(data=data4_students, aes(data4_students$Month.travel)) +
  geom_histogram(bins=24) + 
  geom_freqpoly(bins=12) +
  labs(title = "Histogram showing frequency of \ntravel of students in different months", caption = "Source: xx") +
...
distance_multivariate.OUT<-glm(log(distance_out) ~ data4_with_distance_selected_occupations + as.factor(age_class) + data4_with_distance_selected_genders + data4_with_distance_selected_ReasonTravel +data4_with_distance_selected_MeansTravel + data4_with_distance_selected_BorderPost + pop.HOME + HOME_urban_rural_distance..degrees. + Month.travel, data=data4_with_distance )
distance_multivariate.OUT
summary(distance_multivariate.OUT) 
coef.distance_multivariate.OUT<-coef(distance_multivariate.OUT) 
exp.distance_multivariate.OUT<-exp(coef(distance_multivariate.OUT)) 
confint(distance_multivariate.OUT, level=0.95) 
conf.distance_multivariate.OUT<-exp(confint(distance_multivariate.OUT)) 

travel_prevalence_multivariate.OUT<- glm(falciparum_travel_prevalence ~ data4_noNA_falciparum_prevalence_selected_occupations + data4_noNA_falciparum_prevalence_selected_genders + data4_noNA_falciparum_prevalence_selected_ReasonTravel + data4_noNA_falciparum_prevalence_selected_BorderPost + pop.HOME + HOME_urban_rural_distance..degrees. , data=data4_noNA_falciparum_prevalence)
travel_prevalence_multivariate.OUT
summary(travel_prevalence_multivariate.OUT) 
coef.travel_prevalence_multivariate.OUT<-coef(travel_prevalence_multivariate.OUT) 
exp.travel_prevalence_multivariate.OUT<-exp(coef(travel_prevalence_multivariate.OUT)) 
confint(travel_prevalence_multivariate.OUT, level=0.95) 
conf.travel_prevalence_multivariate.OUT<-exp(confint(travel_prevalence_multivariate.OUT)) 

time_multivariate.OUT<- glm(as.factor(High.risk.month) ~ data4_selected_occupations + data4_selected_ReasonTravel + data4_selected_MeansTravel + data4_selected_BorderPost + HOME_urban_rural_distance..degrees., data=data4, family=binomial(link=logit))
time_multivariate.OUT
summary(time_multivariate.OUT) 
coef.time_multivariate.OUT<-coef(time_multivariate.OUT) 
exp.time_multivariate.OUT<-exp(coef(time_multivariate.OUT)) 
confint(time_multivariate.OUT, level=0.95) 
conf.time_multivariate.OUT<-exp(confint(time_multivariate.OUT)) 
